...
class PostOffice:
    ...
    def extractValidMessageAtomicValues(self, batch_df: DataFrame):
        # explode destroys rows with empty distribution list array
        df = batch_df \
            ...
            .select(lit("ValidObservation").alias("MessageType"),
                    ...
                    explode("DistributionList").alias("RecipientMarketParticipant"),
                    col("Period_Point_Quantity").alias("Quantity"),
                    col("Period_Point_Quality").alias("Quality"),
                    date_format("Period_Point_Time", self.__dateFormat).alias("Time"),
        ...
        # Flatten to create columns "RecipientMarketParticipant_mRID" and "RecipientMarketParticipant_role"
        df = D.flatten_df(df)

        # Make point values appear in a single object in output
        df = df.withColumn("Point", struct("Time", "Quantity", "Quality")) \
               .drop("Time", "Quantity", "Quality")
